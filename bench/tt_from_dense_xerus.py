...
def tt_from_dense(X, max_r, nIter=1):
    for i in range(nIter):
        ttX = xerus.TTTensor(X, 0., max_r)
    return ttX

@timer
def random(shape):
    return xerus.Tensor.random(shape)


def main():
    # command line arguments
    parser = argparse.ArgumentParser(description='benchmark for converting a dense n^d tensor to tensor train format with the xerus library')

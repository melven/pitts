...
def tt_from_dense(X, max_r, nIter=1):
    for i in range(nIter):
        ttX = tl.decomposition.tensor_train(X, max_r)
    return ttX

@timer
def random(shape):
    return tl.random.random_tensor(shape)


def main():
    # command line arguments
    parser = argparse.ArgumentParser(description='benchmark for converting a dense n^d tensor to tensor train format with the tensorly library')

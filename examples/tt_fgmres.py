...
def tt_fgmres(AOp, b, nrm_b, eps=1.e-6, maxIter=20, verbose=True, symmetric=False, adaptiveTolerance=True, maxPrecondRank=9999):
    ...
    def calc_solution():
        ...
        for i in range(len(y)):
            nrm_x = pitts_py.axpby(y[i], Z[i], nrm_x, x, eps)
        return x, nrm_x
    ...
    for j in range(m):
        ...
        pitts_py.copy(V[j], z)
        pitts_py.normalize(z, maxRank=maxPrecondRank)
        Z = Z + [z]
        w = pitts_py.TensorTrain_double(b.dimensions())
        w_nrm = AOp(Z[j], w, delta, maxRank=9999) # maxRank=(j+2)*rank_b)
        ...
        for i in range(j+1):
            ...
            w_nrm = pitts_py.axpby(-H[i,j], V[i], w_nrm, w, delta)
...
if __name__ == '__main__':
    ...
    x, nrm_x = tt_fgmres(AOp, b, nrm_b, maxIter=100, eps=1.e-8, verbose=True, symmetric=False, adaptiveTolerance=True)

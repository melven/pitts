...
// just import the module if we are in module mode and this file is not included from pitts_tensortrain_operator_apply_transposed_dense.cppm
#if defined(PITTS_USE_MODULES) && !defined(EXPORT_PITTS_TENSORTRAIN_OPERATOR_APPLY_TRANSPOSED_DENSE)
import pitts_tensortrain_operator_apply_transposed_dense;
...
// module export
#ifdef PITTS_USE_MODULES
export module pitts_tensortrain_operator_apply_transposed_dense;
...
PITTS_MODULE_EXPORT namespace PITTS
{
  ...
  template void applyT<float>(const TensorTrainOperator<float>& TTOp, const MultiVector<float>& TTx, MultiVector<float>& TTy);
  template void applyT<double>(const TensorTrainOperator<double>& TTOp, const MultiVector<double>& TTx, MultiVector<double>& TTy);
}

...
namespace PITTS
{
  ...
  namespace internal
  {
    ...
    template<typename T>
    std::pair<Tensor2<T>,Tensor2<T>> normalize_svd(const ConstTensor2View<T>& M, bool leftOrthog, T rankTolerance = 0, int maxRank = std::numeric_limits<int>::max(), bool absoluteTolerance = false, bool useFrobeniusNorm = false);
  }

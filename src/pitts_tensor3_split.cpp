...
template std::pair<Tensor2<double>, Tensor2<double>> PITTS::internal::normalize_svd<double>(const ConstTensor2View<double>& M, bool leftOrthog, double rankTolerance, int maxRank);
template std::pair<Tensor2<float>, Tensor2<float>> PITTS::internal::normalize_svd<float>(const ConstTensor2View<float>& M, bool leftOrthog, float rankTolerance, int maxRank);

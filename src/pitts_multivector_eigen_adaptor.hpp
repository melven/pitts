...
namespace PITTS
{
  ...
  auto ConstEigenMap(const MultiVector<T>& mv)
  {
    ...
    return Eigen::Map<const Matrix, Eigen::Aligned128, Eigen::OuterStride<> >(&mv(0,0), mv.rows(), mv.cols(), Eigen::OuterStride<>(mv.rowChunks()*Chunk<T>::size));
  }
  ...
  {
    using Matrix = Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>;
    return Eigen::Map<Matrix, Eigen::Aligned128, Eigen::OuterStride<> >(&mv(0,0), mv.rows(), mv.cols(), Eigen::OuterStride<>(mv.rowChunks()*Chunk<T>::size));
  }
}

...
// just import the module if we are in module mode and this file is not included from pitts_tensortrain_operator_apply_op.cppm
#if defined(PITTS_USE_MODULES) && !defined(EXPORT_PITTS_TENSORTRAIN_OPERATOR_APPLY_OP)
import pitts_tensortrain_operator_apply_op;
...
// module export
#ifdef PITTS_USE_MODULES
export module pitts_tensortrain_operator_apply_op;
...
PITTS_MODULE_EXPORT namespace PITTS
{
  ...
  void apply(const TensorTrainOperator<T>& TTOpA, const TensorTrainOperator<T>& TTOpB, TensorTrainOperator<T>& TTOpC)
  {
    ...
    const int nDim = TTOpA.row_dimensions().size();
    ...
    for(int iDim = 0; iDim < nDim; iDim++)
    {
      ...
      internal::apply_contract_op(TTOpA, TTOpB, TTOpC, iDim, subTOpA, subTOpB, subTOpC[iDim]);
    }
    TTOpC.tensorTrain().setSubTensors(0, std::move(subTOpC));
  }

  // explicit template instantiations
  template void apply<float>(const TensorTrainOperator<float>& TTOp, const TensorTrainOperator<float>& TTx, TensorTrainOperator<float>& TTy);
  template void apply<double>(const TensorTrainOperator<double>& TTOp, const TensorTrainOperator<double>& TTx, TensorTrainOperator<double>& TTy);
}

...
TEST(PITTS_TensorTrain_fromDense_classical, scalar)
{
  ...
  TensorTrain_double TT = PITTS::fromDense_classical(cbegin(scalar), cend(scalar), dimensions);
...
TEST(PITTS_TensorTrain_fromDense_classical, dimension_mismatch)
{
  ...
  data.resize(10);
  EXPECT_THROW(PITTS::fromDense_classical(cbegin(data), cend(data), std::vector<int>{1}), std::out_of_range);
  EXPECT_NO_THROW(PITTS::fromDense_classical(cbegin(data), cend(data), std::vector<int>{2,5}));
  EXPECT_THROW(PITTS::fromDense_classical(cbegin(data), cend(data), std::vector<int>{1,3,7}), std::out_of_range);
}
...
TEST(PITTS_TensorTrain_fromDense_classical, matrix_2d_1x1)
{
  ...
  TensorTrain_double TT = PITTS::fromDense_classical(cbegin(M), cend(M), dimensions);
  ...
  ASSERT_NEAR(7., TT.subTensor(1)(0,0,0), eps);
}
...
TEST(PITTS_TensorTrain_fromDense_classical, matrix_2d_4x5_maxRank)
{
  ...
  {
    // truncated
    TensorTrain_double TT = PITTS::fromDense_classical(cbegin(M), cend(M), dimensions, 1.e-16, 3);
